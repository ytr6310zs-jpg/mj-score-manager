...
2. `.cursor/commands/` に `/clarify` `/design` `/implement` を追加し、既存委任テンプレと整合した定型フローを一発起動できるようにする  
...
- `.cursor/commands/clarify.md` / `design.md` / `implement.md` が存在し、Agent 入力から `/clarify` `/design` `/implement` で呼び出せる内容になっている
- commands の制約が現行ルールと一致する（設計停止、commit/push/PR は明示依頼時のみ）
