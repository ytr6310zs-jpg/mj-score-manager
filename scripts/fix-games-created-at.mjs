...
function signatureForRow(row){
  ...
  const obj = {};
  for(const k of keys){
    // normalize undefined -> null for stable signature
    obj[k] = copy[k] === undefined ? null : copy[k];
  }
...
async function main(){
  ...
  if (!rows || rows.length === 0){
    console.log('No rows found in `games`.');
    return;
  }
  ...
  for(const r of rows){
    ...
    if(!byCreated.has(ca)) byCreated.set(ca, []);
    byCreated.get(ca).push(r);
  }
  ...
  for(const [createdAt, group] of byCreated.entries()){
    ...
    // group by signature (excluding id and created_at)
    ...
    for(const [sig, items] of sigMap.entries()){
      if (items.length > 1){
        ...
        for(const d of dups){
          ...
          // also remove from existing set
          existingCreated.delete(String(d.created_at ?? ''));
        }
        // keep only first in the map
        sigMap.set(sig, [items[0]]);
      }
    }

    // remaining rows with this createdAt after removals
    const remaining = [];
    for(const items of sigMap.values()) remaining.push(...items);
    if (remaining.length <= 1) continue;

    // need to make created_at unique: shift seconds from base
    // deterministic order
    remaining.sort((a,b) => (a.id || 0) - (b.id || 0));
    const baseMs = createdAt ? new Date(createdAt).getTime() : Date.now();

    // Keep first as base, shift others by +1s, +2s, ... ensuring uniqueness
    for(let i = 1; i < remaining.length; i++){
      let candidateMs = baseMs + i * 1000;
      let candidateIso = new Date(candidateMs).toISOString();
      while(existingCreated.has(candidateIso)){
        candidateMs += 1000;
        candidateIso = new Date(candidateMs).toISOString();
      }
  ...
  if (idsToDelete.length > 0){
    console.log('Deleting', idsToDelete.length, 'exact duplicate rows...');
    ...
    console.log('No exact duplicate rows to delete.');
  }

  // perform updates
  if (updates.length > 0){
    console.log('Updating', updates.length, 'rows to make created_at unique...');
    for(const u of updates){
      const { error: upErr } = await supabase.from('games').update({ created_at: u.created_at }).eq('id', u.id);
      if (upErr){
        console.error('Update error for id', u.id, upErr);
        process.exit(1);
      }
      console.log('Updated id', u.id, '->', u.created_at);
    }
  } else {
    console.log('No created_at updates required.');
  }
