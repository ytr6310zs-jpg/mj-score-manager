...
async function main() {
  ...
  if (!SUPABASE_URL || !SUPABASE_KEY) {
    console.error("SUPABASE_URL / SUPABASE key が見つかりません (.env.local または環境変数を確認)");
    process.exit(1);
  }
  ...
  const { data: players, error: playersErr } = await supabase.from("players").select("id,name").in("name", allNames);
  if (playersErr) {
    console.error("players resolve error:", playersErr);
    process.exit(1);
  }
  ...
  if (existingErr) {
    console.error("existing games check error:", existingErr);
    process.exit(1);
  }
  ...
  if (insertRows.length > 0) {
    const insertRes = await supabase.from("games").insert(insertRows).select("id,notes");
    if (insertRes.error) {
      console.error("insert error:", insertRes.error);
      process.exit(1);
    }
    insertedGames = insertRes.data ?? [];
  }
  ...
  let existingYakRows = [];
  if (gameIds.length > 0) {
    const yakRes = await supabase
      .from("yakuman_occurrences")
      .select("game_id,player_id,yakuman_code,yakuman_name,meta")
      .in("game_id", gameIds);
    if (yakRes.error) {
      console.error("existing yakuman check error:", yakRes.error);
      process.exit(1);
    }
    existingYakRows = yakRes.data ?? [];
  }

  const existingYakKeys = new Set();
  for (const row of existingYakRows) {
    const meta = row.meta && typeof row.meta === "object" ? row.meta : null;
    const importKey = meta && "import_key" in meta ? String(meta.import_key ?? "") : "";
    if (importKey) {
  ...
  if (yakumanInsertRows.length > 0) {
    const yakInsertRes = await supabase.from("yakuman_occurrences").insert(yakumanInsertRows);
    if (yakInsertRes.error) {
      console.error("yakuman insert error:", yakInsertRes.error);
      process.exit(1);
    }
  }
