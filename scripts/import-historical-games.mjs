...
function parseYakumanCell(rawText, participants, sourceFile, gameNo) {
    ...
    if (supabaseUrl && supabaseKey) {
      ...
      return {
        ...
        async fetchImportedGames() {
          const { data, error } = await supabase.from("games").select("id,notes").like("notes", `${IMPORT_NOTE_PREFIX}:%`);
  ...
  async function main() {
    ...
    try {
      ...
      const allNames = Array.from(new Set(plannedGames.flatMap((g) => g.entries.map((entry) => entry.player))));
      ...
      for (const game of plannedGames) {
        ...
        if (existingGameId) {
          pendingGames.push({ ...game, existingGameId });
          continue;
        }
        ...
        insertRows.push(row);
        pendingGames.push({ ...game, existingGameId: null });
      }
