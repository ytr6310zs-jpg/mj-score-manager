...
- 前提: 有効なアカウント（メールアドレス／パスワード）および、MFA が必要な場合は設定済みであること
- 手順:
  1. 管理者から案内されたアプリの URL にアクセスします（例: https://mj-score-manager-tfvp.vercel.app/login）。
  2. メールアドレス（またはユーザー名）とパスワードを入力します。
  3. MFA が有効な場合は、認証アプリに表示されるワンタイムパスワードを入力します。
  4. 「ログイン」をクリック（スマートフォンの場合はタップ）します。
  
  ![ログイン画面](images/step-01-login.png)
  
- 期待結果: ダッシュボード（対局一覧等）が表示されます
- 失敗時の対処: パスワードリセットを行い、それでもログインできない場合は管理者へ連絡してください
...
- 手順:
  ...
  3. 情報を修正し、「保存」または「更新」をクリックして完了します。  
...
# 管理メニュー
- 目的: システムに必要な情報の管理を行う
- 前提: 管理者権限を持っていること（表示されるメニューは権限に依存します）
- 手順:
