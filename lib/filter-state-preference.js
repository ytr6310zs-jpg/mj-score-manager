...
function normalizeSharedFilterState(raw) {
...
function readSharedFilterState() {
...
function writeSharedFilterState(state) {
...
function clearSharedFilterState() {
  if (typeof window === 'undefined') return;
  try {
    window.localStorage.removeItem(STORAGE_KEY);
    try { window.dispatchEvent(new CustomEvent('mj:shared-filter-changed', { detail: null })); } catch (e) {}
  } catch (e) {}
}

function buildSharedFilterSearchParams(state, options) {
...
function hasExplicitParams(search, pathname) {
